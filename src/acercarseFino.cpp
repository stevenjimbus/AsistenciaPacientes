          ...
          if (clienteProfundidad.call(servicioProfundidad) && (longitudlistOfPredictions>0))
            {
              ...
              {
                cambioHeadPitch = -5*distanciaHastaElObjeto+25;
                tiempoDeEspera = distanciaHastaElObjeto*1;
...
int main(int argc, char **argv)
{
  ...
  ros::init(argc, argv, "acercamientoFino");
  ROS_INFO("inicio de nodo acercamientoFino");
  ...
  ReceiveDetectedObject_sub = nh_ptr_->subscribe<std_msgs::String>("DetectedObject", 1000, callBack);
  ...
  goGetTheObject = nh_ptr_->advertise<rcnn_live_detector::msgTomarObjeto>("goGrabTheObject", 1);

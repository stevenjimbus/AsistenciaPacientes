...
ros::ServiceServer qr_service;

boost::shared_ptr<ros::NodeHandle> nh_ptr_;
ros::Subscriber searchQR_sub;
...
cv_bridge::CvImageConstPtr takeImageRGBandSave(){
  ...
  sensor_msgs::Image variableWaitForRGBimage= *(ros::topic::waitForMessage<sensor_msgs::Image>("/cv_camera/image_raw", ros::Duration(5)));	
             ...
             rcnn_live_detector::imageTagger::Response &res)
{
	...
	int key = 0;
	if(key != 'q')				// While loop to query for Image Input frame
	{
		...
		if (mark >= 3)		// Ensure we have (atleast 3; namely A,B,C) 'Alignment Markers' discovered
		{
			...
			if( top < contours.size() && right < contours.size() && bottom < contours.size() && cv::contourArea(contours[top]) > 10 && cv::contourArea(contours[right]) > 10 && cv::contourArea(contours[bottom]) > 10 )
			{

				std::vector<cv::Point2f> L,M,O, tempL,tempM,tempO;
				cv::Point2f N;	

				std::vector<cv::Point2f> src,dst;		// src - Source Points basically the 4 end co-ordinates of the overlay image
				...
				*/
				...
				cv::circle( image, M[1], 2,  cv::Scalar(0,255,0), -1, 8, 0 );
				cv::circle( image, O[3], 2,  cv::Scalar(128,128,128), -1, 8, 0 );
				cv::rectangle(traces, M[1], O[3], cv::Scalar(0,0,255), 5, 8, 0);
				std::cout << "BBOX1.x    " << M[1].x << std::endl;
				std::cout << "BBOX1.y    " << M[1].y << std::endl;
				std::cout << "BBOX2.x    " << O[3].x << std::endl;
				std::cout << "BBOX2.y    " << O[3].y << std::endl;
				imwrite( "/home/steven/Desktop/detectedQRbyROS.jpg", image );

				std::vector<double> BBOX;
				BBOX.push_back(M[1].x);
				BBOX.push_back(M[1].y);
				BBOX.push_back(O[3].x);
				BBOX.push_back(O[3].y);

				rcnn_live_detector::Prediction prediccion;
				prediccion.label = "QRcodeGOOD";
				prediccion.score = 1;
				prediccion.bbox = BBOX;

				std::vector<rcnn_live_detector::Prediction> listaDePredicciones;
				listaDePredicciones.push_back(prediccion);

				rcnn_live_detector::PredictionsList lista;
				lista.n = 1;
				lista.predictions = listaDePredicciones;
				res.tags = lista;
			}
		}
		else
		...
		/*
		cv::namedWindow("Traces",cv::WINDOW_NORMAL);
		cv::imshow ( "Traces", traces );
		cv::resizeWindow("Traces", 600, 600);


		cv::namedWindow("QR code",cv::WINDOW_NORMAL);
		cv::imshow ( "QR code", qr_thres );
		cv::resizeWindow("QR code", 300, 300);
...
float cv_lineEquation(cv::Point2f L, cv::Point2f M, cv::Point2f J)
{
	float a,b,c,pdist;
...
float cv_lineSlope(cv::Point2f L, cv::Point2f M, int& alignement)
{
	float dx,dy;
	dx = M.x - L.x;
	dy = M.y - L.y;
...
void cv_getVertices(std::vector<std::vector<cv::Point> > contours, int c_id, float slope, std::vector<cv::Point2f>& quad)
{
	cv::Rect box;
...
{
  ros::init(argc, argv, "servicioDetectQRcode");
  ROS_INFO("Inicio del Nodo: servicioDetectQRcode");  
  ...
  qr_service = nh_ptr_->advertiseService("/qr_service_project_panda", SearchQR);
